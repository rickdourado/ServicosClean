...
async function processarTexto() {
    ...
    try {
        ...
        if (data.sucesso) {
            // Armazena o texto markdown original
            textoMarkdownOriginal = data.resultado;
            
            // Converte Markdown para HTML básico
            const html = converterMarkdownParaHTML(data.resultado);
...
function baixarMarkdown() {
    ...
    const blob = new Blob([textoMarkdownOriginal], { type: 'text/markdown;charset=utf-8' });
